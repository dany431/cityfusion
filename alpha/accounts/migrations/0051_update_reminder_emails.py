...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        accounts = orm.Account.objects.all()
        for account in accounts:
            if account.user.email:
                if not account.reminder_email:
                    account.reminder_email = account.user.email

                if not account.in_the_loop_email:
                    account.in_the_loop_email = account.user.email
                    
                account.save(update_fields=['reminder_email', 'in_the_loop_email'])
                print 'Updating email for user %s' % account.user.username
    ...
    models = {
        ...
        u'event.event': {
            ...
            'created': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2014, 2, 12, 0, 0)', 'auto_now_add': 'True', 'blank': 'True'}),
            'description': ('ckeditor.fields.RichTextField', [], {'blank': 'True'}),
            'email': ('django.db.models.fields.CharField', [], {'max_length': '100'}),
            'event_type': ('django.db.models.fields.CharField', [], {'default': "'SINGLE'", 'max_length': '10'}),
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'location': ('django.contrib.gis.db.models.fields.PointField', [], {}),
            'modified': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2014, 2, 12, 0, 0)', 'auto_now': 'True', 'blank': 'True'}),
