...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        multiday_events = Event.events.filter(event_type="MULTIDAY")

        for event in multiday_events:
            if not event.single_events.filter(is_occurrence=True).count():
                start_time = event.first_occurrence.start_time
                end_time = event.last_occurrence.end_time
                SingleEvent.objects.filter(id__in=event.single_events.all()).update(is_occurrence=True)

                SingleEvent.objects.get_or_create(
                    event_id=event.id,
