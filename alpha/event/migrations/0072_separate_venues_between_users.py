...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        existing_venues = list(orm.Venue.objects.filter(suggested=True))
        events = orm.Event.objects.all()
        created_venues = {}
        for event in events:
            venue = event.venue
            if venue and venue.suggested:
                user = event.owner
                venue_id = venue.id
                if (venue_id, user.id) not in created_venues:
                    new_event_venue = venue
                    new_event_venue.pk = None
                    new_event_venue.user = user
                    new_event_venue.save()

                    created_venues[(venue_id, user.id)] = new_event_venue
                    print 'New venue %s(%s) is created for user %s(%s)' % (new_event_venue.name,
                                                                           venue_id,
                                                                           user.username,
                                                                           user.id)
                else:
                    new_event_venue = created_venues[(venue_id, user.id)]

                event.venue = new_event_venue
                event.save()

        venue_accounts = orm['accounts.VenueAccount'].objects.all()
        for venue_account in venue_accounts:
            ...
            if venue and venue.suggested:
                ...
                venue_id = venue.id
                if (venue_id, user.id) not in created_venues:
                    new_account_venue = venue
                    new_account_venue.pk = None
                    new_account_venue.user = user
                    new_account_venue.save()

                    created_venues[(venue_id, user.id)] = new_account_venue
                    print 'New venue %s(%s) is created for user %s(%s)' % (new_account_venue.name,
                                                                           venue_id,
                                                                           user.username,
                                                                           user.id)
                else:
                    new_account_venue = created_venues[(venue_id, user.id)]

                venue_account.venue = new_account_venue
                venue_account.save()
    ...
    models = {
        ...
        u'event.event': {
            ...
            'created': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2014, 3, 11, 0, 0)', 'auto_now_add': 'True', 'blank': 'True'}),
            'description': ('ckeditor.fields.RichTextField', [], {'blank': 'True'}),
            'email': ('django.db.models.fields.CharField', [], {'max_length': '100'}),
            'event_type': ('django.db.models.fields.CharField', [], {'default': "'SINGLE'", 'max_length': '10'}),
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'location': ('django.contrib.gis.db.models.fields.PointField', [], {}),
            'modified': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2014, 3, 11, 0, 0)', 'auto_now': 'True', 'blank': 'True'}),
