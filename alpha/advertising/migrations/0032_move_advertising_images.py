...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        path_from = os.path.join(settings.MEDIA_ROOT, 'advertising')
        path_to = os.path.join(settings.MEDIA_ROOT, 'rotation')

        shutil.move(path_from, path_to)

        ads = orm.Advertising.objects.all()
        for ad in ads:
            filename = os.path.basename(ad.image.path)
            new_path = os.path.join('rotation', filename)
            ad.image = new_path
            ad.save(update_fields=['image'])


    def backwards(self, orm):
        path_from = os.path.join(settings.MEDIA_ROOT, 'rotation')
        path_to = os.path.join(settings.MEDIA_ROOT, 'advertising')
        ...
        for ad in ads:
            filename = os.path.basename(ad.image.path)
            old_path = os.path.join('advertising', filename)
            ad.image = old_path
            ad.save(update_fields=['image'])
    ...
    models = {
        ...
        u'advertising.advertisingorder': {
            ...
            'budget_currency': ('djmoney.models.fields.CurrencyField', [], {'default': "'CAD'", 'max_length': '3'}),
            'campaign': ('django.db.models.fields.related.ForeignKey', [], {'to': u"orm['advertising.AdvertisingCampaign']"}),
            'created': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2014, 1, 14, 0, 0)', 'auto_now_add': 'True', 'blank': 'True'}),
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
        ...
        u'event.event': {
            ...
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'location': ('django.contrib.gis.db.models.fields.PointField', [], {}),
            'modified': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2014, 1, 14, 0, 0)', 'auto_now': 'True', 'blank': 'True'}),
